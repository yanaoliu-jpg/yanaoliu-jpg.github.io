...
**该查的项**：十二个页面 200 且占位符为零、五组封面等高（只在 >900px 查）、标题和说明起点齐、底边齐、语言切换落在对方语言的同一页（不是跳首页）、**「下一组」首尾相接且不跨语言、自动生成的编号对得上**、中文零缺字形、各设备分辨率档位合适（首页封面和系列页照片都要查）、横向溢出为 0、关掉 JS 渲染结果一致。

上一次跑下来是 **241 项全过**（4 个屏幕尺寸 × 中英双版 + 12 个页面 + 12 条语言切换路径 + 10 条「下一组」链路）。
