...
原因：完整的思源黑体有 18MB，不可能整个塞进网页。脚本只打包你实际用到的字（目前 1553 个，约 409KB）。
第一次跑会把完整字体下到 `素材/字体/`（不进仓库），之后每次都在本地裁，不联网。
**如果加了新字却忘了跑，那几个字会悄悄掉回系统默认字体**，不报错但很难看。
脚本最后会自己检查覆盖率，缺字会直接报错。

### 如果字体脚本报网络错误

比如 `SSL_ERROR_SYSCALL in connection to github.com` —— 那是第一次下完整字体时连 GitHub 的
临时故障（之后都在本地裁，不再联网），**不用慌，也没弄坏任何东西**：脚本下载失败时会先退出，
不会覆盖已有的字体文件。
...
**为什么首页是亮的、内页是深的？**
原来整站深色，理由是第一组照片 91% 的像素亮度在 32 以下。但 2026 年 9 月把十张封面
重新量了一遍：只有第一组还是真的黑，其余九张平均亮度 77、暗部只占 28%——灰调封面放在
深底上不是高级，是浑。所以**首页换成亮的渐变底**，封面浮在上面。
**内页仍然深色**：点进去看全尺寸照片时，暗部细节在白底上会看没。画廊就是这么做的——
白墙挂画，放映厅关灯。内页背景用 `#0d0e11` 而不是纯黑，是为了让画面最暗处仍然和页面有边界。

**字体是什么？**
DM Sans（拉丁）+ 思源黑体 Noto Sans SC（中文），2026 年 9 月从 Newsreader + 思源宋体换的，
为了配首页的亮底和圆角封面。两个都自托管，不向 Google 发任何请求。
